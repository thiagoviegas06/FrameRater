...
const SearchListView = ({ results = [], isPeople = false }) => {
    // Track overlay state for each item using a map of id -> boolean
    const [openOverlayIds, setOpenOverlayIds] = useState({});

    const openOverlay = (id) => {
        setOpenOverlayIds((prev) => ({ ...prev, [id]: true }));
    };

    const closeOverlay = (id) => {
        setOpenOverlayIds((prev) => ({ ...prev, [id]: false }));
    };
    ...
    return (
        <div className="search-list-view">
            {results.map((item) => {
                ...
                const posterUrl = !isPeople && item.poster_path ? `${IMG_BASE}${item.poster_path}` : null;
                const rating = item.vote_average ? Math.round(item.vote_average / 2) : 0;

                const overlayOpen = openOverlayIds[item.id] || false;
                    ...
                    <React.Fragment key={item.id}>
                        ...
                        >
                            ...
                            <div className="list-item-meta">
                                ...
                                <button className="meta-btn" onClick={(e) => e.stopPropagation()}>
                                    <svg
                                        width="14"
                                        height="14"
                                        fill="none"
